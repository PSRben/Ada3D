...
class Norm(nn.Module):
    def __init__(self, in_channels, kernel_size, filter_type,
        ...
        assert filter_type in ('spatial', 'spectral', 'bias', 'none')
...
class CreateSpatialKernel(nn.Module):
    def __init__(self, 
        ...
        self.out_dim = self.spe_channel * (self.kernel_size ** 3)
        self.hidden_dim = int(self.spa_channel * self.spa_se_ratio)

        self.attention = nn.Sequential(
            nn.Conv2d(self.spa_channel, self.hidden_dim, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(self.hidden_dim, self.out_dim, 3, 1, 1)
        )
        self.norm = Norm(self.spe_channel, self.kernel_size, self.norm_type, 'leaky_relu')
...
class CreateAdaBias(nn.Module):
    def __init__(self,
        ...
        self.in_dim = self.spe_channel + self.spa_channel
        self.out_dim = self.spe_channel
        self.hidden_dim = int(self.in_dim * self.channel_se_ratio)

        self.pooling2d = nn.AdaptiveAvgPool2d((1, 1))
        self.pooling3d = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.create_spatial = nn.Conv2d(self.spa_channel, 1, 3, 1, 1)
        self.create_spectral = nn.Conv3d(self.spe_channel, 1, (3, 1, 1), (1, 1, 1), (1, 0, 0))
    ...
    def forward(self, x, y):
        ...
        channel = torch.cat([spe_channel, spa_channel.unsqueeze(2)], dim=1)
...
class Adaptive3D(nn.Module):
    def __init__(self, 
        ...
        self.spectral_kernel = CreateSpectralKernel(self.spe_channel,
                                                    self.spectral_dim,
                                                    self.kernel_size,
                                                    self.spe_se_ratio,
                                                    self.norm)
        self.spatial_kernel = CreateSpatialKernel(self.spe_channel,
                                                  self.spa_channel,
                                                  self.spatial_dim,
                                                  self.kernel_size,
                                                  self.spa_se_ratio,
                                                  self.norm)
        self.bias = CreateAdaBias(self.spe_channel,
                                  self.spa_channel,
                                  self.spectral_dim,
                                  self.kernel_size,
    ...
    def adaptive_3d_conv(self, x):
        ...
        x_unfold = ((x_pad.unfold(-3, k, stride)).unfold(-3, k, stride)).unfold(-3, k, stride)
    ...
    def forward(self, x, y):
        ...
        output = self.adaptive_3d_conv(x) + bias
        return output