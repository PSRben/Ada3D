...
from model.hspansharpening.convnet3d import ConvNet3D as Net
...
def train(args, training_data_loader, validate_data_loader):
    model = Net(
        spe_channel=args.spe_channels,
        spa_channel=args.spa_channels,
        spectral_dim=args.spe_dim,
        spatial_dim=args.spa_dim,
        layer_num=args.layer_num,
        spe_se_ratio=args.beta,
        spa_se_ratio=args.alpha,
        device=args.device
    ).to(args.device)

    summary(model, input_size=[(1, args.spe_dim, 16, 16), (1, args.spa_dim, 64, 64)],
    ...
    for epoch in range(0, args.epoch, 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            gt, pan, ms = batch[0].to(args.device), batch[3].to(args.device), batch[2].to(args.device)
            optimizer.zero_grad()
            sr = model(ms, pan)
            if args.use_ergas is True:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--spa_channels', type=int, default=32, help='Spatial channels')
    parser.add_argument('--spe_channels', type=int, default=8, help='Spectral channels')
    parser.add_argument('--spa_dim', type=int, default=1, help='Spatial data channels')
    parser.add_argument('--spe_dim', type=int, default=191, help='Spectral bands')
    parser.add_argument('--layer_num', type=int, default=3, help='Layer numbers')
    parser.add_argument('--alpha', type=int, default=1, help='Alpha')
    parser.add_argument('--beta', type=int, default=1, help='Beta')
    parser.add_argument('--use_ergas', type=bool, default=True, help='Use ERGAS loss for training or not')
    parser.add_argument('--ergas_hp', type=float, default=3e-5, help='Hyper-parameter for the ERGAS loss')
    parser.add_argument('--epoch', type=int, default=1500, help='Epochs')
    parser.add_argument('--batch_size', type=int, default=2, help='Batch Size')
    parser.add_argument('--lr', type=float, default=5e-4, help='Learning rate')
    parser.add_argument('--step', type=int, default=250, help='Step number')
