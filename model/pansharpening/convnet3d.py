...
# from model.mark.version9.adaptive3d import Adaptive3D
...
class MultiEdge(nn.Module):
    ...
    def forward(self, x):
        ...
        laplace = F.conv2d(x_pad, self.laplace_kernel, stride=1, padding=0)
        sobel_x = F.conv2d(x_pad, self.sobel_kernel_x, stride=1, padding=0)
        sobel_y = F.conv2d(x_pad, self.sobel_kernel_y, stride=1, padding=0)
        sobel = 0.5 * torch.abs(sobel_x) + 0.5 * torch.abs(sobel_y)
        prewitt_x = F.conv2d(x_pad, self.prewitt_kernel_x, stride=1, padding=0)
        prewitt_y = F.conv2d(x_pad, self.prewitt_kernel_y, stride=1, padding=0)
        prewitt = 0.5 * torch.abs(prewitt_x) + 0.5 * torch.abs(prewitt_y)
        scharr_x = F.conv2d(x_pad, self.scharr_kernel_x, stride=1, padding=0)
        scharr_y = F.conv2d(x_pad, self.scharr_kernel_y, stride=1, padding=0)
        scharr = 0.5 * torch.abs(scharr_x) + 0.5 * torch.abs(scharr_y)

        pan_x = torch.zeros([b, 1, h, w], dtype=torch.float, requires_grad=False).to(self.device)
        pan_x[:, :, 1: h, :] = x[:, :, 0: h-1, :]
        pan_x[:, :, 0, :] = x[:, :, 0, :]
        pan_x = torch.abs(pan_x - x)
        pan_y = torch.zeros([b, 1, h, w], dtype=torch.float, requires_grad=False).to(self.device)
        pan_y[:, :, :, 1: w] = x[:, :, :, 0: w-1]
        pan_y[:, :, :, 0] = x[:, :, :, 0]
...
class ResBlockAda3D(nn.Module):
    def __init__(self, dim,
        ...
        super().__init__()
        self.conv0 = Adaptive3D(dim, spectral_dim, spatial_dim, kernel_size,
                                spe_se_ratio, spa_se_ratio, channel_se_ratio)
        self.conv1 = Adaptive3D(dim, spectral_dim, spatial_dim, kernel_size,
                                spe_se_ratio, spa_se_ratio, channel_se_ratio)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
...
class ConvNet3D(nn.Module):
    def __init__(self, 
        ...
        if self.edge is True:
            ...
            nn.Conv2d(7 * self.spatial_dim + self.spectral_dim, self.dim, 3, 1, 1),
                      self.relu
            )
        else:
            self.raise_spa_dim = nn.Sequential(
            nn.Conv2d(self.spatial_dim + self.spectral_dim, self.dim, 3, 1, 1),
                      self.relu
            )
        self.upsample = PixelShuffle(self.spectral_dim, self.scale)
        self.raise_spe_dim = nn.Sequential(
            nn.Conv3d(1, self.dim, 3, 1, 1),
            self.relu
        )
        # self.distribution = nn.Parameter(torch.ones(1, self.layer_num + 1, 1, 1, 1, 1))
        self.retore_dim = nn.Conv3d(self.dim, 1, 3, 1, 1)
        self.refine = nn.Sequential(
            nn.Conv2d(self.spectral_dim, self.spectral_dim, 3, 1, 1),
            self.relu,
            nn.Conv2d(self.spectral_dim, self.spectral_dim, 3, 1, 1)
        )
        ...
        for _ in range(self.layer_num):
            self.spatial_layers.append(ResBlock2D(self.dim, self.res_se_ratio))
            self.spectral_layers.append(ResBlockAda3D(self.dim,
                                                      self.spectral_dim,
                                                      self.spatial_dim,
                                                      self.kernel_size,
                                                      self.spe_se_ratio,
    ...
    def forward(self, x, y):
        ...
        for spe_layer, spa_layer in zip(self.spectral_layers, self.spatial_layers):
            ...
            x = spe_layer(x, y)
            # preserve.append(x)
        # x = torch.stack(preserve, dim=1)
        # x = torch.sum(torch.mul(x, self.distribution), 1) / (self.layer_num + 1)
        # x = x.squeeze(1)
        x = self.retore_dim(x).squeeze(1)
        x = self.refine(x) + x
        return x + skip
